...
const Autocomplete = ({ onSelectItem }) => {
    ...
    useEffect(() => {
        console.log('Autocomplete com ponent did render')
        console.log('timeoutId ', timeoutId)
    });

    const delayedItemsFetch = (inputText) => {
        if (timeoutId) {
            console.log('clearTimeout ', timeoutId)
            clearTimeout(timeoutId);
        }
        const timeOutId = setTimeout(() => {
            console.log('delayedItemsFetch ', inputText);

            fetchItems(inputText, setItems, setLoading);
